...
export async function buildSite(profile, options = {}) {
  ...
  if (profile.site.host === 'netlify') {
    await writeText(path.join(outDir, 'netlify.toml'), netlifyToml());
  }
  ...
  const selfPostingForm = form.provider === 'netlify';
  if (selfPostingForm) {
    warnings.push('The contact form uses Netlify Forms, so it only captures submissions once deployed to Netlify.');
  } else if (!form.action && !profile.contact.email) {
...
/**
 * Netlify site config: long-lived caching for fingerprint-free assets is unsafe,
 * so images get a year and HTML is always revalidated.
 */
function netlifyToml() {
  return `# Generated by Avo.
[build]
  publish = "."
