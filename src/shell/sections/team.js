...
${items.map((m) => `  <li class="card card--person">
    ${m.photo ? `<div class="card__media card__media--portrait">${image(asset(m.photo), m.name, { className: 'card__img', sizes: '(min-width: 900px) 25vw, 50vw', variants: ctx.variantsFor?.(m.photo) ?? null })}</div>` : `<span class="card__icon">${icon('users')}</span>`}
