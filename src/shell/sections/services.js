...
</div>${service.image ? `\n<div class="service-detail__media">${image(asset(service.image), service.name, { className: 'rounded', sizes: '(min-width: 900px) 50vw, 100vw', variants: ctx.variantsFor?.(service.image) ?? null })}</div>` : ''}`,
  });
}
